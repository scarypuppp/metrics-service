...
Были оптимизированы middleware'ы: compress и decompress:

До оптимизации на каждый запрос создавался новый объект gzip.Writer для compress и gzip.Reader для decompress.
Само создание этих объектов ленивое — память аллоцируется при первой записи/чтении.

Для оптимизации эти объекты вынесены в пул sync.Pool и переиспользуются между запросами, вместо того чтобы создаваться 
заново на каждый запрос. Пул может держать несколько экземпляров одновременно — по одному на воркер при параллельной 
нагрузке, — так что дорогая аллокация буферов происходит лишь несколько раз, а не на каждый запрос.
Перед использованием объект ресетится (gz.Reset(w) для writer'а, gz.Reset(r.Body) для reader'а), 
что переиспользует уже выделенные буферы для нового запроса.

Также был оптимизирован handler, отдающий список метрик в html:

До оптимизации был цикл по всем метрикам, где итоговая строка строилась конкатенацией. На каждой итерации она
копировалась целиком заново, что давало O^2 сложность по памяти. Для оптимизации я использовал
`strings.Builder` с `Grow()`. Строки пишутся в объект StingBuilder с помощью fmt.Fprintf() и StingBuilder.WriteString.
